...
- [ ] Keep the ChatGPT Codex backend compatibility headers current with the official Codex CLI.
  ...
  - The builtin Codex model list (`gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra` in crates/lam-tui/src/config.rs `builtin_codex_provider`) and the 1,050,000 context window are also static mirrors: the window is copied from the openai-responses entries, and whether the subscription backend honors the full window (or counts reasoning tokens against it) is unverified. Empirically validate with a long session; server-side overflow surfaces as a clean context-length 400, not silent truncation.
