...
- [ ] Keep the ChatGPT Codex backend compatibility headers current with the official Codex CLI.
  ...
  - The native `lam-agent login openai` OAuth parameters (scope `openid profile email offline_access`, audience `https://api.openai.com/v1`, ephemeral loopback redirect port) mirror the official codex-cli's known behavior but have only been tested against local mock listeners; live-verify one end-to-end login and adjust the constants at the top of crates/lam-tui/src/codex.rs if the authorization server rejects them.
